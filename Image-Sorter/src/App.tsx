...
type GroupOption = 'none' | 'folder' | 'type'
...
const sortItems = (items: MediaItem[], sortBy: SortOption) => {
...
function App() {
  ...
  const groupedItems = useMemo(() => {
    if (groupBy === 'none') {
      return [{ title: 'Alle Medien', items: filteredItems }]
    }

    const grouped = filteredItems.reduce<Record<string, MediaItem[]>>((acc, item) => {
      const key = groupBy === 'folder' ? item.folder : item.type === 'image' ? 'Fotos' : 'Videos'
      if (!acc[key]) acc[key] = []
      acc[key].push(item)
      return acc
    }, {})

    return Object.entries(grouped).map(([title, sectionItems]) => ({
      title,
      items: sectionItems,
    }))
        ...
        <div className="section">
          ...
          <div className="filter-group">
            <label>
              ...
              <select
                value={filters.range}
                onChange={(event) => setFilters((prev) => ({ ...prev, range: event.target.value as Filters['range'] }))}
              >
            ...
            <label>
              ...
              <select
                value={filters.size}
                onChange={(event) => setFilters((prev) => ({ ...prev, size: event.target.value as Filters['size'] }))}
              >
            ...
            <label>
              ...
              <select
                ...
                onChange={(event) => setFilters((prev) => ({ ...prev, flagged: event.target.value as Filters['flagged'] }))}
              >
                <option value="all">Alle</option>
      ...
      <div className="main-area">
        <header className="topbar">
          ...
          <div className="actions">
            ...
            <button className="primary" onClick={() => setShowDeleteModal(true)} disabled={deleteCandidates.length === 0}>
              Auswahl prüfen
            </button>
          </div>
        </header>
            ...
            <div className="grid-controls">
              ...
              <label>
                ...
                <select value={groupBy} onChange={(event) => setGroupBy(event.target.value as GroupOption)}>
                  ...
                  <option value="type">Medientyp</option>
                </select>
              </label>
              <label>
                Layout
            ...
            <div className="grouped-grid">
              {groupedItems.map((group) => (
                <div key={group.title} className="group-block">
                  {groupBy !== 'none' ? <h3>{group.title}</h3> : null}
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal-header">
              <h2>Auswahl prüfen</h2>
              <button className="ghost" onClick={() => setShowDeleteModal(false)}>
                Schließen
              </button>
            </div>
            <p>
              {deleteCandidates.length} Dateien markiert · {formatBytes(deleteSize)} Speicherfreigabe
            </p>
