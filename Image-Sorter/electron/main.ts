...
const toMediaUrl = (filePath: string) => `${mediaScheme}://${encodeURIComponent(filePath)}`
...
app.whenReady().then(() => {
  protocol.registerFileProtocol(mediaScheme, (request, callback) => {
    const url = request.url.replace(`${mediaScheme}://`, '')
